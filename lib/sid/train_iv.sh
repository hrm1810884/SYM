...
bash $cut_wavbin $wavspklist $cutwavspklist ${bin_path}/tmp
...
python $calc_ivector $(wc -l ${bin_path}/tmp/mfcc.txt | cut -d ' ' -f 1) 3200 ${bin_path}/tmp/svs_bin ${bin_path}/wmatrix/wmatrix.npy > ${bin_path}/tmp/train_tmp.dat
...
python3 ${calc_ivector} ${bin_path}/tmp/mfcc.txt $mu_ubm $sigma_ubm $w_ubm $TVmatrix > ${bin_path}/tmp/train_tmp.dat
paste -d ' ' ${bin_path}/tmp/speaker.txt ${bin_path}/tmp/train_tmp.dat > ${bin_path}/tmp/train.dat
